...
HDusage_by_subject_bar_plot <- function(table_summary){
  ...
  p <-  ggplot(table_summary, aes(Subject, HD39_percent*100)) + 
	  ...
	  scale_y_continuous(breaks=c(0,20,40,60),labels=c('0','20','40','60'),expand=c(0,0),limits=c(0,60)) +
	  scale_fill_manual(values=c('gray30','gray70'),drop=FALSE) +
	  ylab(bquote(bold('D3-9 gene segment usage (%)'))) +
	  xlab(bquote(bold('')))
  ggsave('graph/HD39_Andrews_SciImmun_Patient_usage.png',p,height=2.5,width=3)
  }

table_summary <- read_tsv('result/Dgene_Andrews_SciImmun_Patient_summary.tsv') %>%
