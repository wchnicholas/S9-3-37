...
def parse_Andrews_patient_file(filename):
  ...
  for line in infile.xreadlines():
    if line[0:4] == 'Subj': continue
    line  = line.rsplit("\t")
    name  = '_'.join(line[0:2])
    Vgene = line[2]
    Dgene = line[3]
    CDRH3 = line[5]
    CDRH3_len = len(CDRH3)
...
def analyze_Dgene(Dgene_dict, patientID, outfile_HD39, outfile_Dgene):
  ...
  for name in Dgene_dict.keys():
    if patientID != name.rsplit('_')[0]: continue
    Total_count += 1
...
def main():
  filename = 'data/Andrews_STM_StemAbList.tsv'
  outfile_HD39  = 'result/HD39_Andrews_STM_Patient_summary.tsv'
  outfile_Dgene = 'result/Dgene_Andrews_STM_Patient_summary.tsv'
  ...
  patientIDs = list(set(([ID.rsplit('_')[0] for ID in Dgene_dict.keys()])))
